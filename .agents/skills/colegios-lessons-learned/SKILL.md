...
## Overview
Use this skill to repeat the working setup for Prisma migrations/seed, and Coolify/VPS deployment details (networks, env vars, and OpenSSL fixes).
> **IMPORTANT**: We are no longer working locally. All deployments are launched to the VPS using the Docker Compose configuration located at `G:\codex_projects\colegios\compose`.

## Workflow (quick)
1. VPS compose: Use the deploy file at `G:\codex_projects\colegios\compose` for all operations.
...
## VPS/Coolify Notes
- Canonical deploy file: `G:\codex_projects\colegios\compose` (this is the single source of truth for all current work).
