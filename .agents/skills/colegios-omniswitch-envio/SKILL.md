...
Usar esos scripts para confirmar nombres de endpoints, campos de payload y orden exacto. No inventar variantes del## Scripts de Pruebas y Credenciales
Existe un script de pruebas utilitario en `services/api/scripts/omni-send-debug.mjs` que sirve para disparar un envío masivo de prueba con 2 PDFs.
> **WARNING**: Este script local contiene datos *hardcodeados* del firmante (cédula, nombres, celular, etc.) para acelerar pruebas de desarrollo y validación.
> Funciona ejecutando:
> ```bash
> cd G:\codex_projects\colegios\services\api
> npm run omni:debug-send -- "ruta_pdf_1.pdf" "ruta_pdf_2.pdf"
> ```
> Las credenciales de OmniSwitch para este script se leen de `.env` (ej. `OMNISWITCH_USER`, `OMNISWITCH_PASSWORD`). Asegúrate de no persistir credenciales reales de producción directamente en el código de la aplicación.

## Flujo Básico (4 Pasos)mpararlas contra esa referencia.
